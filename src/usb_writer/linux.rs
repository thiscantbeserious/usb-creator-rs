...
impl UsbWriter for LinuxUsbWriter {
    fn list_devices() -> Result<Vec<UsbDisk>, UsbWriterError> {
        ...
        let output = Command::new("sh")
            ...
            .output()
            .map_err(|_| UsbWriterError::CommandExecutionError("Failed to execute lsblk".to_string()))?;
            ...
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(output_str) {
                if let Some(blocks) = parsed["blockdevices"].as_array() {
                    for blk in blocks {
                        ...
                        let vendor = blk["vendor"].as_str().unwrap_or_default().trim().to_string();
                        let model = blk["model"].as_str().unwrap_or_default().trim().to_string();
                        let mountpoint = blk["mountpoint"].as_str().map(|s| s.to_string());
                        let size = Byte::parse_str(&blk["size"].as_str().unwrap_or_default().replace(",", "."), true).map(|b| b.as_u64()).unwrap_or(0);
                        devices.push(UsbDisk { name: format!("/dev/{}", name), vendor, model, mountpoint, size, blocktype });
                    }
                }
            }
        }
